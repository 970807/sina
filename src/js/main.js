...
$(function(){
	...
	$(".product .mod-prev").click(function(){
		...
		$(".product>div").animate({left:"+=253"},200,function(){
			if(!animateFlag){
				...
				let $last = $(".product>div").eq(3).clone(false);
	...
	$(".product .mod-next").click(function(){
		...
		$(".product>div").animate({left:"-=253"},200,function(){
			if(!animateFlag){
				...
				let $last = $(".product>div").eq(0).clone(false);
	...
	$(".heimaogif").hover(function(){
		$(this).children("img").attr("src", require("../images/blackcat-hover.gif"));
	},function(){
		$(this).children("img").attr("src", require("../images/blink.gif"));
	});
	/* 新浪产品-end */
...
function adsCarousel(){
	let timer = setInterval(function(){
		...
		$(".ads>div").animate({top:"-=50"},1000,function(){
			if(n++ == 0){
				let $img = $(".ads>div").eq(0).clone(false);
				$(".ads").append($img);
				$(".ads>div").eq(0).remove();
			}
			if(n == 3){
